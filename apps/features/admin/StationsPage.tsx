...
export function StationsPage({ stations, campuses, buildings }: Props) {
  ...
  const [editStation, setEditStation] = useState<StationWithRelations | undefined>();
      ...
      <div className="flex items-center justify-between">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{editStation ? "給水機を編集" : "給水機を追加"}</DialogTitle>
            </DialogHeader>
            <StationForm
              campuses={campuses}
              buildings={buildings}
        ...
        <Table>
          ...
          <TableBody>
            ...
            {stations.map((station) => (
              <TableRow key={station.id} className={!station.isPublic ? "opacity-50" : ""}>
                <TableCell className="font-medium">{station.name}</TableCell>
                <TableCell>{station.campus.name}</TableCell>
                <TableCell>{station.building.name}</TableCell>
                <TableCell>
                  <Badge variant={STATUS_BADGE[station.status] as "default" | "secondary" | "destructive"}>
                ...
                <TableCell>
                  <div className="flex gap-1 flex-wrap">
                    {station.temperatures.map((t) => (
                      <Badge key={t.temperatureType} variant="outline" className="text-xs">
                    ...
                    {station.isPublic && (
                      <AlertDialog>
                        <AlertDialogTrigger asChild>
                          <Button size="sm" variant="secondary">非公開化</Button>
                        </AlertDialogTrigger>
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            <AlertDialogTitle>非公開化しますか？</AlertDialogTitle>
                            <AlertDialogDescription>
                              「{station.name}」を非公開にします。QRコードが設置済みの場合はこちらを推奨します。
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
                            <AlertDialogCancel>キャンセル</AlertDialogCancel>
                            <AlertDialogAction onClick={() => handleUnpublish(station.id)}>
                              非公開にする
                            </AlertDialogAction>
                          </AlertDialogFooter>
                        </AlertDialogContent>
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            「{station.name}」を完全に削除します。この操作は取り消せません。
                            {station.shortLinkUrl && " QRコードと紐づいているため、非公開化を検討してください。"}
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                          <AlertDialogCancel>キャンセル</AlertDialogCancel>
